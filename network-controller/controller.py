...
class Controller(RyuApp):
    ...
    def packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if elapsed_time > 60 and dst_mac not in self.whitelist:
                if dst_mac not in self.guest_list:
                    self.guest_list.append(dst_mac)
                    self.logger.info(f"Device {dst_mac} marked as suspicious")
            else:
                self.whitelist.append(dst_mac)
                self.logger.info(f"Device {dst_mac} whitelisted")

            actions = [] if dst_mac in self.guest_list else [parser.OFPActionOutput(out_port), parser.OFPActionSetQueue(0)]
    ...
    def request_stats_periodically(self, datapath):
        ...
        threading.Timer(60, self.update_guest_list).start()

    def update_guest_list(self):
        try:
            payload = {"guest_list": self.guest_list}
            requests.post("http://127.0.0.1:8000/update_guest_list", json=payload)
        except requests.exceptions.RequestException as e:
            print(f"Error sending guest list to the API: {e}")

    def get_guest_list(self):
        try:
            response = requests.get("http://127.0.0.1:8000/get_guest_list")
            if response.status_code == 200:
                temp_list = response.json().get("guest_list")
                if temp_list:
                    self.guest_list = temp_list
        except Exception as e:
            print(f"Error sending guest list to the API: {e}")
    ...
    def whitelist_device(self, datapath, mac):
        try:
            ...
            self.logger.info(f"GUEST LIST: {self.guest_list}")
            ...
            self.logger.info(f"Deleting existing flow rule for device {mac}")
